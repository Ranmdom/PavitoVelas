...
// Componente com Motion e delay configurável
export default function ProductCard({
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    addItem({ id, name, price, image, weight, color })
  }
        ...
        <Link href={`/produto/${id}`} className="block">
          <div className="relative aspect-square overflow-hidden bg-[#FBE1D0]/70">
            <div
              ...
              style={{ background: `radial-gradient(circle, ${color}40 0%, ${color}10 70%)` }}
            />
            <Image
          ...
          <CardFooter className="flex gap-2 p-4 pt-0 bg-white/60 backdrop-blur-sm">
            ...
            <Button
              variant="ghost"
              className="flex-shrink-0 text-[#631C21] hover:bg-[#631C21]/10 hover:text-[#631C21]"
            >
              Ver
            </Button>
          </CardFooter>
